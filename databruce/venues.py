...
def get_city_id(city_name: str, cur: psycopg.Cursor) -> int | None:
...
def get_state_id(state: str, cur: psycopg.Cursor) -> int | None:
    ...
    try:
        state_abbrev = res.fetchone()
        return state_abbrev["id"]
    except TypeError:
        return None


def get_country_id(country_name: str, cur: psycopg.Cursor) -> int | None:
...
def get_continent_by_country(country: int, cur: psycopg.Cursor) -> int | None:
...
def venue_name_fix(venue_name: str) -> str:
...
def venue_parser(
    ...
    fixed_name = venue_name_fix(venue_name)
    ...
    if match := re.search(r"^(.*),\s(.*),\s(.*)(?:,.+)?$", venue_name):
        ...
        venue["city"] = get_city_id(matches[1], cur)
        venue["country"] = get_country_id(matches[-1], cur)
        ...
        if matches[-1] == "Australia":
            ...
            venue["city"] = get_city_id(match_split[1], cur)
            venue["state"] = get_state_id(matches[-2], cur)

        # US state abbreviations are two chars long
        if re.search(r"^\w{2}$", matches[-1]):
            venue["state"] = get_state_id(matches[-1].upper(), cur)

            # uses the states table and grabs the proper country name
            country = get_country_from_abbrev(
                matches[-1],
                cur,
            )

            # finds the proper country id given the country
            venue["country"] = get_country_id(country, cur)

        # getting continent based on country
        venue["continent"] = get_continent_by_country(
            venue["country"],
            cur,
        )
...
def get_venues(cur: psycopg.Cursor, client: httpx.Client) -> None:
    """Get a list of venues from Brucebase, and inserts into the database."""
    response = scraper.post("17778201", client)

    if response:
        soup = bs4(response, "lxml")
        links = html_parser.get_clean_links(soup, "/venue:")

    venues = [venue_parser(link["text"], link["url"], cur) for link in links]

    res = cur.execute("""SELECT distinct brucebase_url FROM venues""")

    # not the ideal way to do this. But, without this check, it will
    # insert every single url whether there is a detail or not
    existing = [v["brucebase_url"] for v in res.fetchall()]
